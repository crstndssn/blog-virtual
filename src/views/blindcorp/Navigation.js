...
const Navigation = () => {
    ...
    useEffect(() => {
        ...
        auth.onAuthStateChanged((user) => {
            ...
            if (user.email === 'contador@blindcorp.net') {
                setAdmin(true)
                setUser(false)
            } else {
                setUser(true)
    ...
    const CerrarSesion = () => {
        auth.signOut()
        setUsuario(null)
        history.push('/blindcorpdecolombia/login')
    }


    return (
        <nav className="container mx-auto flex justify-between items-center py-8">
            <Link className="w-full md:text-3xl xs:text-xl font-semibold xs:hidden md:block tracking-tighter" to="/blindcorpdecolombia">Blog Virtual</Link>
            {
                usuario ?
                    (
                        <div className="flex md:justify-center xs:justify-between items-center xs:w-full md:w-auto">
                            {
                                admin === true ? (
                                    <div className="flex md:justify-center xs:justify-between items-center xs:w-full md:w-auto">
                                        <Link to="/blindcorpdecolombia/info" id="addPostBtn" className="only-admin-add">
                                            <img src={infoSvg}
                                                alt="add" className="w-7 md:mx-6 xs:mx-0" />
                                        </Link>
                                        <Link exact to="/blindcorpdecolombia/posts" id="addPostBtn" className="only-admin-add">
                                        ...
                                        {/* <Link to="/blindcorpdecolombia/notifications" className="only-admin-notification">
                                    ...
                                    <div className="flex md:justify-center xs:justify-between items-center xs:w-full md:w-auto">
                                        <Link exact to="/blindcorpdecolombia/info" id="addPostBtn" className="only-admin-add">
                                            <img src={infoSvg}
                                                alt="add" className="w-7 md:mx-6 xs:mx-0" />
                                        </Link>
                                        <Link to="/blindcorpdecolombia/posts" id="addPostBtn" className="only-admin-add">
                                            <img src="https://firebasestorage.googleapis.com/v0/b/supersociedadeselkindussan.appspot.com/o/filesPosts%2F33wkaxK10KMLn7q6m3ZpjWZK57n1%2Fhome.svg?alt=media&token=421fa765-f95b-4f98-bbbe-8e4b37c7ca8e"
                                                alt="add" className="w-7 md:mx-6 xs:mx-0" />
                                        </Link>
                                        <p onClick={CerrarSesion} >
                    ...
                    (
                        <div className="w-full flex justify-end items-center">
                            <Link to="/blindcorpdecolombia/signup" className="cursor-pointer font-medium md:text-xl xs:text-lg mx-6 logged-out"
                                id="btn-signup">Sign Up</Link>
                            <Link to="/blindcorpdecolombia/login"
